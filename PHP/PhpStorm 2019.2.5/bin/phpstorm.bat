...
:: Try (in order): PHPSTORM_JDK, phpstorm%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=

IF EXIST "%PHPSTORM_JDK%" SET JDK=%PHPSTORM_JDK%
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.PhpStorm2019.2\config\phpstorm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.PhpStorm2019.2\config\phpstorm%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ...
  ECHO No JDK found. Please validate either PHPSTORM_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: explicit
SET VM_OPTIONS_FILE=%PHPSTORM_VM_OPTIONS%
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.PhpStorm2019.2\config\phpstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_PHPSTORM_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_PHPSTORM.hprof" -Didea.paths.selector=PhpStorm2019.2 %IDE_PROPERTIES_PROPERTY%
SET IDE_JVM_ARGS=-Didea.platform.prefix=PhpStorm
